...
class DependenciesHandler {
 public:
  ...
  [[nodiscard]] auto get_affected_positions(const CellPos &pos) const noexcept
      -> const std::optional<std::unordered_set<CellPos>> {
    if (!is_in_dependencies(pos, m_dependencies)) return std::nullopt;
 ...
 private:
  ...
  static auto catch_circling_cells_DFS(  // TODO: IMPLEMENT
      const Dependencies &deps, std::unordered_set<CellPos> visited) noexcept
